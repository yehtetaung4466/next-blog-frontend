...
export default function UserProfile({ id }: { id: number }) {
  ...
  const posts = blogs ? (
    ...
    <div className=" h-full text-2xl font-medium text-gray-400  flex justify-center items-center">
      No post by {inspect ? username : 'you'}
    </div>
  );
  const atvs =
    activities && activities?.length !== 0 ? (
      ...
      <div className=" h-full text-2xl font-medium text-gray-400  flex justify-center items-center">
        No recent activity by {inspect ? username : 'you'}
      </div>
    );
  useEffect(() => {
    try {
          ...
          <div className=" flex w-full h-full">
            ...
            <div className=" w-1/2 h-full flex flex-col items-start justify-start">
              <div className=" ml-5 mt-3 font-medium">
                ...
                <div className=" dropdown dropdown-end">
                  <label
                    ...
                    className={` ${
                      inspect ? 'hidden' : null
                    } cursor-pointer ml-1 relative bottom-1`}
                  >
                    ...
                  </label>
                  <ul
              ...
              <div className=" ml-5 text-sm sm:text-base">
                since:{' '}
                {user
                  ? DateTime.fromISO(user.createdAt).toFormat('yyyy.M.dd')
                  : null}
              </div>
              <span className=" ml-3">
                <Back />
        ...
        <div className="row-span-1 col-span-12 flex">
          ...
          <span className="tabs tabs-boxed w-fit h-full">
            <Link
              ...
              prefetch={true}
              replace={true}
              className={`tab ${activeTab === 'posts' ? 'tab-active' : null}`}
            >
              posts
            </Link>
            <Link
              href={`/profile/${id}?tab=activity`}
              prefetch={true}
              replace={true}
              className={`tab ${
                activeTab === 'activity' ? 'tab-active' : null
              }`}
            >
              activity
            </Link>

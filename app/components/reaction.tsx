...
export default function Reaction({blog,currentUser}:{blog:Blog,currentUser:number|undefined}) {
  ...
  const deleteReaction = async()=>{
    const res = await fetch(`${process.env.NEXT_PUBLIC_NEST_SERVER}/api/reactions`,{
      method:"DELETE",
      headers: {
        'Authorization':`Bearer ${Cookies.get("access_token")}`,
        'Content-Type': 'application/json',
    ...
    const initialReaction:Reaction = blog.reactions.find((r)=>r.author_id===currentUser)?.reaction||'neutral'; 
    const reactionReducer = (currentReaction:Reaction,action:Action):Reaction=>{
        switch(action.type) {
            case "like":
    ...
    const likeFilled = (
        <div onClick={()=>dispatch({type:"like"})} className="z-50 reaction_switch fill-blue-500 ">
          <svg
            role="filled"
      ...
      const likeUnfilled = (
        <div onClick={()=>dispatch({type:"like"})}  className="z-50 reaction_switch">
          <svg
            role="unfilled"
            xmlns="http://www.w3.org/2000/svg"
            height="24"
        ...
        const dislikeFilled = (
            <div onClick={()=>dispatch({type:"dislike"})} className=" reaction_switch fill-red-500 ">
              <svg
                role="filled"
                xmlns="http://www.w3.org/2000/svg"
                height="24"
          ...
          const dislikeUnfilled = (
            <div onClick={()=>dispatch({type:"dislike"})}  className=" reaction_switch">
              <svg
                role="unfilled"
                xmlns="http://www.w3.org/2000/svg"
                height="24"
